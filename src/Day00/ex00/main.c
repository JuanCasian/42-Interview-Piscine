...
int main(void)
{
	...
	num_art_pieces = 94888; //Counted the art pieces

	// test of art pieces that are there
	for (int i = 0; i < 10; i++)
	{
		rand_num = rand() % num_art_pieces;
		printf("price for the art \'%s\' is %d\n", arts[rand_num]->name, searchPrice(arts, arts[rand_num]->name));
	}
	// test of art piece that does not exits
	if (searchPrice(arts, "Do not exist") == -1)
